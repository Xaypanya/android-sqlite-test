...
public class myDatabase extends SQLiteOpenHelper {
    ...
    public long deleteData(String bid){
        try {
            SQLiteDatabase db =this.getWritableDatabase();
//            String sql="Update tbbook set bname=?,price=?,page=? where bid=?";
            String sqlquery="delete from tbbook where bid=?";
            SQLiteStatement stm = db.compileStatement(sqlquery);
            stm.bindString(1,bid);
    ...
    public Cursor searchData(String keyword){
        try {
            SQLiteDatabase db = this.getReadableDatabase();
            String sql="select * from tbbook where bname=?";
            SQLiteStatement stm = db.compileStatement(sql);
            stm.bindString(1, keyword);
            Cursor cur = db.rawQuery(sql,null);
